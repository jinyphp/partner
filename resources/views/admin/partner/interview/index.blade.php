                            ...
                            <div class="col-md-4">
                                ...
                                <select name="result" class="form-select">
                                    @foreach($filterOptions['results'] as $value => $label)
                                        <option value="{{ $value }}" {{ $currentFilters['result'] === $value ? 'selected' : '' }}>
                                            {{ $label }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div class="col-md-4">
                                ...
                                <select name="type" class="form-select">
                                    @foreach($filterOptions['types'] as $value => $label)
                                        <option value="{{ $value }}" {{ $currentFilters['type'] === $value ? 'selected' : '' }}>
                                            {{ $label }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div class="col-md-4">
                                ...
                                <select name="date_range" class="form-select">
                                    @foreach($filterOptions['date_ranges'] as $value => $label)
                                        <option value="{{ $value }}" {{ $currentFilters['date_range'] === $value ? 'selected' : '' }}>
                                            {{ $label }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div class="col-md-4">
                                ...
                                <select name="interviewer" class="form-select">
                                    ...
                                    @foreach($interviewers as $interviewer)
                                        <option value="{{ $interviewer->id }}" {{ $currentFilters['interviewer'] == $interviewer->id ? 'selected' : '' }}>
                                            {{ $interviewer->name }}
                                        </option>
                                    @endforeach
                                </select>
            ...
            <div class="card">
                <div class="card-header d-flex justify-content-between align-items-center">
                    ...
                    <div class="d-flex align-items-center">
                        <select name="per_page" onchange="location.href='{{ request()->fullUrlWithQuery(['per_page' => '']) }}'+(this.value)" class="form-select form-select-sm me-2" style="width: auto;">
                        ...
                        <select name="sort_by" onchange="location.href='{{ request()->fullUrlWithQuery(['sort_by' => '', 'sort_direction' => $currentFilters['sort_direction']]) }}'+(this.value)" class="form-select form-select-sm" style="width: auto;">
                                ...
                                <tr>
                                    ...
                                    <td>
                                        @if($interview->interviewer)
                                            <div class="fw-medium">{{ $interview->interviewer->name }}</div>
                                        @else
                                    ...
                                    <td>
                                        <div class="small">
                                            <div>{{ $interview->type_label }}</div>
                                            <div class="text-muted">{{ $interview->round_label }}</div>
                                        </div>
                                    </td>
                                    <td>
                                        @if($interview->scheduled_at)
                                            <div class="small">
                                                <div>{{ $interview->scheduled_at->format('Y-m-d') }}</div>
                                    ...
                                    <td>
                                        ...
                                        @if($interview->interview_result)
                                            <div class="mt-1">
                                                ...
                                                @endphp
                                                <span class="badge bg-{{ $resultColor }}">{{ $interview->result_label }}</span>
                                            </div>
                                        @endif
                                    </td>
                                    <td>
                                        @if($interview->overall_score)
                                            <div class="text-center">
                                                <div class="fw-medium">{{ $interview->overall_score }}</div>
                                    ...
                                    <td>
                                        <div class="btn-group btn-group-sm">
                                            <a href="{{ route('admin.partner.interview.show', $interview->id) }}" class="btn btn-outline-primary">보기</a>
                                            @if(!in_array($interview->interview_status, ['completed']))
                                                <a href="{{ route('admin.partner.interview.edit', $interview->id) }}" class="btn btn-outline-secondary">수정</a>
                                            @endif
                                        </div>
                                    </td>
                                </tr>
                    ...
                    <div class="card-body p-0">
                        @foreach($todayInterviews as $interview)
                            <div class="d-flex align-items-center p-3 border-bottom">
                                ...
                                <div>
                                    <a href="{{ route('admin.partner.interview.show', $interview->id) }}" class="btn btn-sm btn-outline-primary">보기</a>
                                </div>
                            </div>
                        @endforeach
                    </div>
