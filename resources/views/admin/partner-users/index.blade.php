...
@section('content')
    <div class="container-fluid">
                        ...
                        <div class="d-flex align-items-center">
                            ...
                            <div class="flex-grow-1 ms-3">
                                ...
                                <h3 class="mb-0 fw-bold">
                                    <span class="text-success">{{ $statistics['active'] ?? 0 }}</span> /
                                    {{ $statistics['total'] ?? 0 }}
                                </h3>
        ...
        <!-- 필터 -->
        @includeIf("jiny-partner::admin.partner-users.filter")
                    ...
                    <div class="table-responsive">
                        <table class="table table-hover mb-0">
                            <thead class="table-light">
                                <tr>
                                    <th >회원 정보</th>
                                    <th width="150">수수료율</th>

                                    <th width="200">총매출액 / 총커미션</th>
                                    <th width="100">작업 / 평점</th>
                                    <th width="200">가입일 / 테이블</th>
                                    <th width="100">관리</th>
                                </tr>
                            </thead>
                            <tbody>
                                    ...
                                    <tr>
                                        <td>
                                            <div>
                                                <div class="mb-1">
                                                    ...
                                                    <!-- 파트너 타입 -->
                                                    @if ($item->partnerType)
                                                        <span class="badge bg-primary ms-2">{{ $item->partnerType->type_name }}</span>
                                                    @else
                                                        <span class="badge bg-secondary ms-2">타입 미지정</span>
                                                    @endif
                                                    <!-- 파트너 등급 -->
                                                    <span class="badge bg-info ms-1">{{ $item->partnerTier->tier_name ?? 'N/A' }}</span>
                                                </div>
                                                <div class="mb-2">
                                        ...
                                        <td>
                                            <div class="text-center">
                                                ...
                                                <!-- 구성 요소별 상세 -->
                                                <div class="mt-1">
                                                    @if ($item->partnerType && $item->partnerType->default_commission_rate > 0)
                                                        <small class="text-muted d-block">타입:
                                                            {{ $item->partnerType->default_commission_rate }}%</small>
                                                    @endif
                                                    @if ($item->partnerTier && $item->partnerTier->commission_rate > 0)
                                                        <small class="text-muted d-block">등급:
                                                            {{ $item->partnerTier->commission_rate }}%</small>
                                                    @endif
                                                    @if ($item->individual_commission_type === 'percentage' && $item->individual_commission_rate > 0)
                                                        <small class="text-muted d-block">개인:
                                                            {{ $item->individual_commission_rate }}%</small>
                                                    @elseif($item->individual_commission_type === 'fixed_amount' && $item->individual_commission_amount > 0)
                                                        <small class="text-muted d-block">개인:
                                                            {{ number_format($item->individual_commission_amount) }}원</small>
                                                    @endif
                                                </div>
                                            </div>
                                        </td>
                                        ...
                                        <td>
                                            <div class="text-end">
                                                @php
                                                    $stats = $item->realtime_stats ?? [];
                                                    $totalSales = $stats['total_sales'] ?? 0;
                                                    $totalCommissions = $stats['total_commissions'] ?? 0;
                                                    $teamSales = $stats['team_sales'] ?? 0;
                                                    $balance = $stats['commission_balance'] ?? 0;
                                                    $commissionRate =
                                                        $totalSales > 0
                                                            ? round(($totalCommissions / $totalSales) * 100, 1)
                                                            : 0;
                                                    $isCached = $stats['cached'] ?? false;
                                                @endphp

                                                <!-- 총 매출액 -->
                                                <div
                                                    class="fw-bold text-success d-flex align-items-center justify-content-end">
                                                    <i class="fe fe-trending-up me-1 text-success"></i>
                                                    {{ number_format($totalSales) }}원
                                                    @if ($isCached)
                                                        <small class="text-info ms-1" title="캐시된 데이터"><i
                                                                class="fe fe-clock"></i></small>
                                                    @endif
                                                </div>

                                                <!-- 팀 매출액 (있는 경우) -->
                                                @if ($teamSales > $totalSales)
                                                    <small class="text-muted">팀:
                                                        {{ number_format($teamSales) }}원</small><br>
                                                @endif

                                                <!-- 총 커미션 -->
                                                <div
                                                    class="fw-bold text-primary d-flex align-items-center justify-content-end mt-1">
                                                    <i class="fe fe-dollar-sign me-1 text-primary"></i>
                                                    {{ number_format($totalCommissions) }}원
                                                    @if ($commissionRate > 0)
                                                ...
                                                @if ($balance != 0)
                                                    <div
                                                        class="small mt-1 {{ $balance > 0 ? 'text-warning' : 'text-danger' }}">
                                                        <i class="fe fe-wallet me-1"></i>
                                                        잔액: {{ number_format($balance) }}원
                                                    </div>
                                                @endif
                                            </div>
                                        </td>
                                        <td>
                                            <div class="text-center">
                                                <!-- 완료 작업 (첫 번째 줄) -->
                                                <div
                                                    class="fw-bold text-info d-flex align-items-center justify-content-center">
                                                ...
                                                <!-- 평점 (두 번째 줄) -->
                                                <div
                                                    class="fw-bold text-warning d-flex align-items-center justify-content-center mt-1">
                                        ...
                                        <td>
                                            <div class="text-center">
                                                <!-- 가입일 (첫 번째 줄) -->
                                                <div
                                                    class="fw-bold text-dark d-flex align-items-center justify-content-center">
                                                ...
                                                <div class="d-flex align-items-center justify-content-center mt-1">
                                                    ...
                                                    <code
                                                        class="bg-light px-2 py-1 rounded text-sm">{{ $item->user_table }}</code>
                                                </div>
                                            </div>
                                        </td>
                                        <td>
