...
class StoreController extends Controller
{
     ...
     * 파트너 타입 생성 처리
     *
     * 마이그레이션 구조에 맞춘 완전한 유효성 검증 및 데이터 처리
     * - 모든 27개 필드 지원
     * - JSON 필드 자동 처리
     * - 체크박스 필드 올바른 변환
     * - 수수료 타입별 조건부 검증
    ...
    public function __invoke(Request $request)
    {
        ...
        $validated['partner_tiers_count'] = 0; // 초기값

        try {
            $item = $this->model::create($validated);

            return redirect()->route("admin.{$this->routePrefix}.show", $item->id)
                ->with('success', "{$this->title} '{$item->type_name}'이 성공적으로 생성되었습니다.");
        } catch (\Exception $e) {
            return back()
                ->withInput()
                ->with('error', '파트너 타입 생성 중 오류가 발생했습니다: ' . $e->getMessage());
        }
    }

    /**
     * JSON 필드 처리 (specialties, required_skills)
     */
    ...
    protected function getValidationRules(Request $request): array
    {
        $rules = [
            ...
            'type_code' => [
                ...
                'max:20',
                'regex:/^[a-z0-9_]+$/',
                Rule::unique('partner_types', 'type_code')
            ],
